document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      var invalids = contactForm.querySelectorAll('.invalid');
        ...
        if (field.hasAttribute('required') && value === '') {
          ...
          if (field.hasAttribute('maxlength')) {
            ...
            if (value.length > maxLen) {
              fieldValid = false;
              errorMsg = 'Maximum ' + maxLen + ' characters allowed';
            }
          }
          if (field.hasAttribute('pattern')) {
            ...
            if (!pattern.test(value)) {
              fieldValid = false;
              errorMsg = 'Please match the requested format';
            }
          }
        }
      ...
      if (isValid) {
        ...
        // Simulate submission - replace with actual fetch/AJAX as needed
        setTimeout(function() {
          contactForm.reset();
          if (submitBtn) {
  ...
  if (animatedElements.length > 0) {
    ...
    var observer = new IntersectionObserver(function(entries) {
      for (var i = 0; i < entries.length; i++) {
        if (entries[i].isIntersecting) {
          entries[i].target.classList.add('visible', 'animated');
    ...
    for (var m = 0; m < animatedElements.length; m++) {
      ...
      animatedElements[m].style.transform = 'translateY(20px)';
      animatedElements[m].style.transition = 'opacity 0.6s ease-out, transform 0.6s ease-out';
      observer.observe(animatedElements[m]);
    }

    // Add CSS rule for visible state
    var style = document.createElement('style');
    style.textContent = '.visible.animated, .fade-in.visible, .animate-on-scroll.visible { opacity: 1 !important; transform: translateY(0) !important; }';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '021707af-fbc0-415c-9145-1ef08efc628b',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '021707af-fbc0-415c-9145-1ef08efc628b';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '021707af-fbc0-415c-9145-1ef08efc628b';
